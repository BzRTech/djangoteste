...
const DescriptorCatalog = () => {
  ...
  const fetchDescriptors = async () => {
    try {
      ...
      const uniqueSubjects = [...new Set(descriptorsArray.map(d => d.subject).filter(Boolean))];
      const uniqueGrades = [...new Set(descriptorsArray.map(d => d.grade).filter(Boolean))];
      const uniqueFields = [...new Set(descriptorsArray.map(d => d.learning_field).filter(Boolean))];
