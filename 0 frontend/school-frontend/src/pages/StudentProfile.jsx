...
const StudentProfile = () => {
  ...
  const fetchProfile = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_BASE_URL}/student-profile/${id}/profile/`);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 text-lg">Carregando perfil do aluno...</p>
        </div>
      </div>
    );
  }
  ...
  const competencyRadarData = profile.recent_progress.map(p => ({
  ...
  const descriptorsBySubjectData = Object.entries(profile.descriptors.by_subject).map(([subject, data]) => ({
    subject: subject,
    total: data.total,
      ...
      <div className="max-w-7xl mx-auto">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-4 text-gray-600">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span>{profile.class_name} - {profile.school_name}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Calendar className="w-5 h-5" />
                    <span>Série: {profile.grade}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <StatCard
            ...
            value={profile.descriptors.achieved_count}
            total={profile.descriptors.total_count}
            icon={Target}
            color="blue"
          />
          <StatCard
            title="Taxa de Conquista"
            value={`${profile.descriptors.percentage}%`}
            icon={TrendingUp}
            color="green"
          />
          <StatCard
            title="Provas Realizadas"
            value={profile.recent_exams.length}
            icon={Award}
            color="purple"
          />
          <StatCard
            title="Última Avaliação"
            value={profile.recent_progress.length > 0 
