...
import {
  Clock, CheckCircle, AlertCircle, FileText, BookOpen,
  ChevronRight, ChevronLeft, Send, Trophy, XCircle,
  Loader2, Target, Award, Home
} from 'lucide-react';
...
const TakeExam = () => {
  ...
  const loadAvailableExams = async () => {
    try {
      ...
      const available = applications.filter(app =>
        app.status === 'in_progress' || app.status === 'scheduled'
      );
  ...
  const submitExam = async () => {
    ...
    try {
      ...
      // Enviar respostas
      const response = await fetch(`${API_BASE_URL}/student-answers/bulk_create/`, {
        method: 'POST',
        headers: {
        ...
        <div className="max-w-4xl mx-auto">
          <div className="bg-white rounded-xl shadow-lg p-8">
            <div className="text-center mb-8">
              ...
              <h1 className="text-3xl font-bold text-gray-800 mb-2">Fazer Prova</h1>
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      <h3 className="font-semibold text-gray-800">{studentData.student_name}</h3>
                      <p className="text-sm text-gray-500">Matrícula: {studentData.student_serial}</p>
                    </div>
            ...
            <div className="bg-gray-50 p-6 rounded-lg mb-6">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <span className="font-semibold ml-2">{result.total_score} / {result.max_score}</span>
                </div>
                <div>
                  <span className="text-gray-600">Tempo:</span>
                  <span className="font-semibold ml-2">{formatTime(timeElapsed)}</span>
                </div>
                <div>
                  <span className="text-gray-600">Questões respondidas:</span>
                  <span className="font-semibold ml-2">{Object.keys(answers).length} / {questions.length}</span>
                </div>
                <div>
                  <span className="text-gray-600">Status:</span>
                  <span className={`font-semibold ml-2 ${passed ? 'text-green-600' : 'text-yellow-600'}`}>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 mb-6 sticky top-6 z-10">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h1 className="text-2xl font-bold text-gray-800">{selectedExam.exam_name}</h1>
            ...
            <div className="mb-2">
              <div className="flex justify-between text-sm text-gray-600 mb-1">
                <span>Progresso</span>
                <span>{answeredCount} de {totalQuestions} questões</span>
              </div>
            ...
            <div className="lg:col-span-3">
              <div className="bg-white rounded-xl shadow-lg p-8">
                  ...
                  <div className="flex items-center justify-between mb-4">
                    ...
                    {currentQuestion.difficulty_level && (
                      <span className={`text-sm font-semibold px-3 py-1 rounded-full ${
                        ...
                        {currentQuestion.difficulty_level === 'medium' && 'Médio'}
                        {currentQuestion.difficulty_level === 'hard' && 'Difícil'}
                      </span>
                    )}
                  </div>
                ...
                <div className="space-y-3 mb-8">
                  {currentQuestion.alternatives && currentQuestion.alternatives.map((alt) => {
                      ...
                      >
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1">
                            ...
                            <span className="text-gray-800">{alt.alternative_text}</span>
                          </div>
                        </div>
                      </button>
                    );
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-lg p-8 mb-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-4xl font-bold text-gray-800 mb-2">Provas Disponíveis</h1>
              <p className="text-gray-600">Olá, {student?.student_name}! Selecione uma prova para começar.</p>
            </div>
            <button
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl shadow-lg p-12 text-center">
            ...
            <h2 className="text-2xl font-bold text-gray-800 mb-2">Nenhuma prova disponível</h2>
            <p className="text-gray-600">Não há provas liberadas para você no momento.</p>
          </div>
        ) : (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                    ...
                    {exam.status === 'in_progress' ? 'Em Andamento' : 'Agendada'}
                  </span>
                </div>
                ...
                <div className="space-y-2 text-sm text-gray-600 mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span>{new Date(exam.application_date).toLocaleDateString('pt-BR')}</span>
                  </div>
                  {exam.start_time && (
                    <div className="flex items-center gap-2">
                      <Clock className="w-4 h-4" />
                      <span>{exam.start_time} - {exam.end_time || 'Sem limite'}</span>
                    </div>
                  )}
                </div>
