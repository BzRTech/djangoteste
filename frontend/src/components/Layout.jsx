...
const Layout = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800 flex transition-colors duration-300">
      {/* Sidebar - Desktop */}
      <aside
        className={`hidden md:flex flex-col bg-white dark:bg-gray-800 shadow-xl transition-all duration-300 ${
      ...
      >
        ...
        <div className="py-4 px-3 border-t border-gray-200 dark:border-gray-700">
          <button
            ...
            className={`flex items-center gap-3 px-3 py-2.5 rounded-lg font-medium transition-all whitespace-nowrap w-full text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-800 dark:hover:text-white ${
              sidebarCollapsed ? "justify-center" : ""
            }`}
            title={sidebarCollapsed ? (darkMode ? "Tema Claro" : "Tema Escuro") : ""}
          >
            {darkMode ? (
            ...
            {!sidebarCollapsed && <span>{darkMode ? "Tema Claro" : "Tema Escuro"}</span>}
          </button>
        </div>
      </aside>

      {/* Mobile Header */}
      <div className="md:hidden fixed top-0 left-0 right-0 bg-white dark:bg-gray-800 shadow-lg z-50 transition-colors duration-300">
        <div className="flex items-center justify-between h-16 px-4">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Profile Button - Mobile (always visible) */}
            <NavLink
              to="/profile"
              className={({ isActive }) =>
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        {/* Profile Button - Desktop (Fixed position, always visible) */}
        <div className="hidden md:flex fixed top-4 right-4 z-50 items-center gap-2">
          <button
            onClick={toggleDarkMode}
            className="p-2.5 bg-white dark:bg-gray-800 rounded-lg shadow-lg text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
          ...
          <NavLink
            to="/profile"
            className={({ isActive }) =>
              `flex items-center gap-2 px-4 py-2.5 rounded-lg font-medium shadow-lg transition-all ${
                isActive
                  ? "bg-blue-600 text-white"
